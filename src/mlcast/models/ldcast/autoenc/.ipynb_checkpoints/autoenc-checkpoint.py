...
class AutoencoderKL(pl.LightningModule):
    ...
    def val_test_step(self, batch, batch_idx, split="val"):
        ...
        self.log(f"{split}_loss", total_loss, **log_params, sync_dist=True)
        self.log(f"{split}_rec_loss", rec_loss.mean(), **log_params, sync_dist=True)
        self.log(f"{split}_kl_loss", kl_loss, **log_params, sync_dist=True)
